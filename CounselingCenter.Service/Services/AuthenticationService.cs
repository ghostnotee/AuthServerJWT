    ...
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<Response<TokenDto>> CreateTokenByRefreshTokenAsync(string refreshToken)
        {
            var existRefreshToken =
                await _userRefreshTokenService.Where(urt => urt.Code == refreshToken).SingleOrDefaultAsync();
        ...
        {
            var existRefreshToken = await
                _userRefreshTokenService.Where(urt => urt.Code == refreshToken).SingleOrDefaultAsync();
        ...
        public Response<ClientTokenDto> CreateTokenByClient(ClientLoginDto clientLoginDto)
        {
            ...
            if (client is null) return Response<ClientTokenDto>.Fail("ClientId or Secret not found", 404, true);
